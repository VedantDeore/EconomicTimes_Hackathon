...
interface HRASplit { recommended_claimer: string; savings: number; reason: string }
interface InsurancePlan { joint_vs_individual: string; recommendations: string[] }
interface CombinedNetWorth { total: number; partner_1_share: number; partner_2_share: number }
interface Optimization {
  hra_split?: HRASplit;
  insurance_plan?: InsurancePlan;
  combined_net_worth?: CombinedNetWorth;
}
interface CoupleData { optimization?: Optimization }
...
export default function CouplesPlannerPage() {
  ...
  const sendInvite = async () => {
    try {
      const res = await api.post("/couples/invite", { partner_email: partnerEmail });
  ...
  const fetchOptimizations = async () => {
    setLoading(true);
    try {
      const res = await api.get("/couples/optimize");
      setData(res.data);
      setTab("results");
    } catch { /* handle */ }
    ...
    <div className="space-y-8 max-w-5xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">India&apos;s first AI-powered joint financial planning tool</p>
        </div>
      </div>
      ...
      {tab === "invite" && (
        <div className="p-6 rounded-2xl bg-slate-800/40 border border-slate-700/50 space-y-4">
          ...
          <p className="text-sm text-slate-500">Your partner needs to have an ET Finance account. Enter their email to send an invite.</p>
          <div className="flex gap-3">
            <input type="email" value={partnerEmail} onChange={(e) => setPartnerEmail(e.target.value)}
              placeholder="partner@example.com"
              className="flex-1 px-4 py-2.5 rounded-xl bg-slate-900/60 border border-slate-700/50 text-white text-sm focus:outline-none focus:border-violet-500/50" />
          ...
          {inviteCode && (
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
              <p className="text-sm text-emerald-400">✅ Invite sent! Share this code: <span className="font-mono font-bold">{inviteCode}</span></p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === "results" && !opt && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-500">Link with your partner to see joint financial optimizations</p>
          <button onClick={fetchOptimizations} disabled={loading}
          ...
          {opt.combined_net_worth && (
            <div className="p-6 rounded-2xl bg-slate-800/40 border border-slate-700/50">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                <Wallet size={18} className="text-emerald-400" /> Combined Net Worth
              </h3>
          ...
          {opt.hra_split && (
            <div className="p-5 rounded-2xl bg-slate-800/40 border border-slate-700/50">
              ...
              <p className="text-lg font-bold text-emerald-400 mt-2">
                Save {formatCurrency(opt.hra_split.savings)}
              </p>
            </div>
          )}

...
function getMonthlyExpenseTotal(m: FinancialProfile["monthly_expenses"] | undefined): number {
  ...
  return Object.entries(m).filter(([k]) => k !== "total").reduce((s, [, v]) => s + (typeof v === "number" ? v : 0), 0);
}
...
function getLocalMentorResponse(
  ...
  if (lower.includes("sip") || lower.includes("systematic")) {
    ...
    return {
      ...
      tool_result: { monthly_sip: monthly, horizon_years: 15, illustrative_corpus: Math.round(corpus) },
  ...
  if (lower.includes("tax") || lower.includes("regime")) {
    if (analysis) {
      ...
      return {
        ...
        tool_result: { recommended: rc.recommended_regime, old_tax: rc.old_regime.total_tax, new_tax: rc.new_regime.total_tax, savings: rc.savings },
  ...
  if (lower.includes("insurance") || lower.includes("cover")) {
    const lifeCover = typeof profile?.insurance?.life?.sum_assured === "number" ? Number(profile.insurance.life.sum_assured) : 0;
    ...
    return {
      ...
      tool_result: { target_life: targetLife, current_life: lifeCover, gap: Math.max(0, targetLife - lifeCover) },
  ...
  return {
    response: "I can help with **SIPs, tax regimes, insurance, FIRE planning, and asset allocation**. What would you like to explore?",
    tool_used: null, tool_result: null,
    suggestions: ["Calculate my SIP", "Compare tax regimes", "Asset allocation advice", "Check my insurance gap"],
  };
}
  ...
  return text.split("\n").map((line, j) => {
    ...
    return (
      <p key={j} className={j > 0 ? "mt-1.5" : ""}>
        {parts.map((part, k) => k % 2 === 1 ? <strong key={k} className="font-semibold text-white">{part}</strong> : <span key={k}>{part}</span>)}
      </p>
    );
  });
}
...
function ToolResultCard({ data, toolUsed }: { data: Record<string, unknown>; toolUsed: string | null }) {
  ...
  return (
    <div className="mt-2 rounded-lg border border-cyan-500/20 bg-slate-950/60 p-3 text-xs shadow-inner shadow-emerald-500/5">
      {toolUsed && <div className="mb-2 font-mono text-[10px] uppercase tracking-wide text-cyan-400/90">{toolUsed}</div>}
      <dl className="space-y-1.5">
        {entries.map(([key, val]) => (
          <div key={key} className="flex justify-between gap-3 border-b border-slate-700/40 pb-1.5 last:border-0 last:pb-0">
            ...
            <dd className="text-right text-slate-200">{typeof val === "number" ? val.toLocaleString("en-IN") : String(val)}</dd>
          </div>
        ))}
      </dl>
    </div>
...
const ALGO_SECTIONS = [
  {
    ...
    description: "Groq Mixtral classifies your question, selects a calculator (SIP, Tax, Insurance, Asset Allocation, XIRR), extracts parameters from natural language, executes the computation, then formats results conversationally.",
  },
  {
    title: "Context-Aware Pipeline",
    description: "Your financial profile and latest tax analysis are automatically injected as context, so responses are personalized without you re-entering data.",
  },
];
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const aMsg: ChatMessage = {
        role: "assistant", content: payload.response, timestamp: new Date(),
        tool_used: payload.tool_used, tool_result: payload.tool_result ?? undefined,
        suggestions: payload.suggestions ?? [],
      };
      setMessages((prev) => [...prev, aMsg]);
      void saveChatMessage("assistant", payload.response, payload.tool_used ?? undefined, payload.tool_result ?? undefined);
    } catch {
      setMessages((prev) => [...prev, { role: "assistant", content: "Sorry, something went wrong. Please try again.", timestamp: new Date(), suggestions: ["Calculate my SIP", "Compare tax regimes"] }]);
    } finally {
      setIsTyping(false);
    }
  }, [profile, analysis]);
        ...
        <div className="w-64 shrink-0 border-r border-slate-700/50 bg-slate-900/50 p-4 hidden lg:flex flex-col">
          <div className="flex items-center gap-2 mb-6">
            ...
            <div>
              ...
              <p className="text-[11px] text-emerald-400/90">Context-aware mentor</p>
            </div>
          </div>

          <div className="mb-4">
            <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500 mb-2">Quick Actions</p>
            <div className="space-y-1">
              {QUICK_ACTIONS.map((label) => (
                <button key={label} type="button" onClick={() => void sendMessage(label)}
                  ...
                  <Zap size={12} className="inline mr-1.5 text-cyan-400" />{label}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4">
            <div className="max-w-3xl mx-auto space-y-4">
              {messages.map((msg, i) => (
                <div key={`${msg.timestamp.getTime()}-${i}`} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="flex items-center gap-1 rounded-2xl border border-slate-700/50 bg-slate-800 px-4 py-3">
                    {[0, 1, 2].map((d) => (
                      <motion.span key={d} className="h-2 w-2 rounded-full bg-emerald-400"
          ...
          <div className="border-t border-slate-700/50 bg-slate-900/60 px-4 md:px-8 py-3">
            <form onSubmit={(e) => { e.preventDefault(); void sendMessage(input); }} className="max-w-3xl mx-auto">
              ...
              <p className="mt-2 text-center text-[9px] text-slate-600">Educational information only. Consult a qualified professional for personal advice.</p>
            </form>
          </div>
        </div>
      </div>

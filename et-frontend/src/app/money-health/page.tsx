...
export default function MoneyHealthPage() {
  ...
  const calculateScore = async () => {
    ...
    try {
      if (localMode) {
        setReport(computeHealthReport(mergeHealthInputs(derived, tweaks)));
        return;
      }
      const res = await api.post<HealthReport>("/health/calculate");
      setReport(res.data);
    } catch {
      setError("Could not calculate score. Check your connection or try local mode (default).");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Six dimensions — local mode uses your saved profile + sliders below</p>
          </div>
        </div>
        <button
      ...
      {localMode && (
        <div className="p-6 rounded-2xl bg-slate-800/40 border border-slate-700/50 space-y-4">
          <h3 className="text-sm font-semibold text-white">Quick adjustments (local)</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <label className="block">
              <span className="text-slate-500 text-xs">Emergency (months)</span>
              <input
                type="range"
                min={0}
                max={12}
                value={tweaks.emergency_months ?? derived.emergency_months}
                onChange={(e) =>
                  setTweaks((t) => ({ ...t, emergency_months: Number(e.target.value) }))
                }
                className="w-full mt-1"
              />
            </label>
            <label className="block">
              <span className="text-slate-500 text-xs">Debt / income % (annual)</span>
              <input
                type="range"
                min={0}
                max={80}
                value={tweaks.debt_to_income_pct ?? derived.debt_to_income_pct}
                onChange={(e) =>
                  setTweaks((t) => ({ ...t, debt_to_income_pct: Number(e.target.value) }))
                }
                className="w-full mt-1"
              />
            </label>
            <label className="block">
              <span className="text-slate-500 text-xs">Investment buckets used</span>
              <input
                type="range"
                min={1}
                max={8}
                value={tweaks.investment_types_count ?? derived.investment_types_count}
                onChange={(e) =>
                  setTweaks((t) => ({ ...t, investment_types_count: Number(e.target.value) }))
                }
                className="w-full mt-1"
              />
            </label>
      ...
      {!report && !loading && (
        <div className="text-center py-20">
          ...
          <p className="text-slate-600 max-w-md mx-auto mb-6">
            {localMode
              ? "Your profile is loaded from this device. Adjust sliders if needed, then calculate."
              : "Complete your financial profile and click Calculate Score."}
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {HEALTH_DIMENSIONS.map((dim) => {
              ...
              return (
                <div
                  key={dim.key}
                  className="p-5 rounded-2xl bg-slate-800/40 border border-slate-700/50 hover:border-slate-600/50 transition-all"
                >
            ...
            <div className="p-6 rounded-2xl bg-slate-800/40 border border-slate-700/50">
              ...
              <div className="space-y-3">
                {report.top_3_actions.map((a, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-4 p-4 rounded-xl bg-slate-900/40 border border-slate-700/30"
                  >
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-r from-pink-500/10 to-rose-500/10 border border-pink-500/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-pink-400">Summary</span>
            </div>
